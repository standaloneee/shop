...
public class SaleController {
    ...
    // Насколько я понял группа товаров == список товаров для которых надо сделать скидку, т.к.
    @PutMapping("/")
    @PreAuthorize("@authService.authInfo.hasRole('ADMIN')")
    public ResponseEntity<Set<Product>> editProductSalesByTags(@PathVariable @RequestBody Set<Tag> tags, short discount){
        return new ResponseEntity<>(saleService.editProductSalesByTags(tags, discount), HttpStatus.OK);
    }
